                ...
                <div class="bg-white rounded-md shadow overflow-x-auto">
                    <table class="w-full whitespace-nowrap">
                        <tr class="text-left font-bold">
                            ...
                            <th class="pb-4 pt-6 px-6">Frontend Type</th>
                            <th class="pb-4 pt-6 px-6">Is Required</th>
                            <th class="pb-4 pt-6 px-6">Is Filterable</th>
                        </tr>
                        <tr v-for="attribute in props.attributes" :key="attribute.id" class="hover:bg-gray-100 focus-within:bg-gray-100">
                            <td class="border-t">
                                <Link class="flex items-center px-6 py-4 focus:text-indigo-500" :href="route('admin.attributes.edit', {attribute: attribute})">
                                    {{ attribute.name }}
                            ...
                            <td class="border-t">
                                <icon :name="attribute.is_required"></icon>
                            </td>
                            <td class="border-t">
                                <icon :name="attribute.is_filterable"></icon>
                            </td>
                            <td class="w-px border-t">
                                <Link class="flex items-center px-4" :href="route('admin.attributes.edit', {attribute: attribute})" tabindex="-1">
...
watch(data, throttle(() => {
    Inertia.get('attributes', pickBy(data.form), {preserveState: true})
}, 1000), {
    deep: true
})
